...
# 获取企业微信应用推送 QYWX_Server
if "QYWX_Server" in os.environ:
    if len(os.environ["QYWX_Server"]) > 1:
        QYWX_Server = os.environ["QYWX_Server"]


if "QYWX_KEY" in os.environ:
    if len(os.environ["QYWX_KEY"]) > 1:
        QYWX_KEY = os.environ["QYWX_KEY"]
        # print("已获取并使用Env环境 QYWX_Server")
...
def wecom_app(title, content):
    try:
        if not QYWX_Server:
            print("QYWX_Server 并未设置！！\n取消推送")
            return
        QYWX_AM_AY = re.split(',', QYWX_Server)
        if 4 < len(QYWX_AM_AY) > 5:
            print("QYWX_Server 设置错误！！\n取消推送")
            return
        corpid = QYWX_AM_AY[0]
        ...
        if not media_id:
            response = wx.send_news(title, content, touser)
        else:
...
class WeCom:
    ...
    def send_news(self, title, message, touser="@all"):
